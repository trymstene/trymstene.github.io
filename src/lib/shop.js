...
import PRINTFUL_HEX from '../data/colour-hex.json';
...
// ⭐ THE REAL SOURCE IS PRINTFUL. `colour-hex.json` is generated by
// `node tools/build-colour-hex.js` from every catalog the slate sells, so a new
// colourway arrives with its own hex instead of a grey dot. The map above is
// now only a FALLBACK for names Printful does not publish a code for (its own
// mugs are one), and for anything the codegen has not run for yet.
// ⚠️ A miss is silent by design — grey is a legal colour — so the tool is what
// catches it: run it after every product batch. It found Agave, Harbor Blue and
// Carbon Grey all rendering grey on the long sleeve within a day of launch.
export const colorHex = (name) => {
  const k = (name || '').trim().toLowerCase();
  return PRINTFUL_HEX[k] || COLOR_HEX[k] || '#cccccc';
};
