...
const SEEDS = [
  ...
  // 🌵 THE EXOTIC TIER — gardener lvl 5 (30 harvests): the ladder the top
  // gardeners ran out of. Six stars is a TIER marker, not inflation — the
  // wilt window (2+⌊6/2⌋ = 5 dry days) and bloom feed scale with it, which
  // is exactly what week-long plants need. The grapevine regrows like the
  // strawberry — the daily loop, top-shelf edition.
  { id: 'grape', emoji: '🍇', name: 'grapevine', stars: 6, price: 400, days: 6, crop: 1, regrow: 3 },
  { id: 'pineapple', emoji: '🍍', name: 'pineapple', stars: 6, price: 500, days: 8, crop: 1, rare: true },
  { id: 'prickly', emoji: '🌵', name: 'prickly pear', stars: 6, price: 650, days: 9, crop: 1, rare: true },
];
const SEED_BY = {};
...
// GLVL_AT / GLVL_STARS live in pass-defs.js now — the HUD chip and the
// homestead read the same table
...
const STAGE_ART = {
  ...
  grape1: ['c-grape-1.png', 31, 63], grape2: ['c-grape-2.png', 33, 63],
  grape3: ['c-grape-3.png', 33, 63], grape4: ['c-grape-4.png', 38, 69],
  pineapple1: ['c-pineapple-1.png', 26, 19], pineapple2: ['c-pineapple-2.png', 26, 26],
  pineapple3: ['c-pineapple-3.png', 26, 33], pineapple4: ['c-pineapple-4.png', 33, 65],
  prickly1: ['c-prickly-1.png', 26, 24], prickly2: ['c-prickly-2.png', 28, 40],
  prickly3: ['c-prickly-3.png', 28, 44], prickly4: ['c-prickly-4.png', 38, 79],
...
export function initGarden(ctx) {
  ...
  // ⚠️ NEVER FROZEN AT BOOT — the world-gid can land mid-load (a pass pull
  // finishing just after the park starts), and a stale pair disowns everything
  // gFetch stamps from that moment on. gFetch resets it from the ids it sends;
  // me() re-reads at most once a second, because it sits in render loops.
  let myShort = worldOwner().slice(0, 8);
  let myAlt = worldSid().slice(0, 8);
  let idAt = Date.now();
  function me() {
    const t = Date.now();
    if (t - idAt > 1000) {
      idAt = t;
      myShort = worldOwner().slice(0, 8);
      myAlt = worldSid().slice(0, 8);
    }
    return myShort;
  }
  ...
  async function gFetch(path, body) {
    ...
    // 🪪 …and the cached pair rides along, so the client's sense of ownership
    // is exactly the id the room is about to write (see me()).
    const own = worldOwner(), sid = worldSid();
  ...
  // ⚠️ BOTH NAMES, EVERYWHERE — the same rule as the worker's isMine. Plots
  ...
  function gardenerLvl() {
    return gardenerLvlFor((passGet().stats || {}).garden_harvests || 0);
  }
  // 🧑‍🌾 THE GARDENER CARD — the HUD chip's tap target: the whole ladder in
  // one card, current level marked, next rung named. Panel-first (the
  ...
  ctx.gardenerCard = openGardenerCard;   // the HUD chip taps in through the seam
  ...
  async function doHarvest(i) {
    ...
    // gardener credit and toast the pick while the plant stood in the room
    if (!gDone(res)) {
    ...
    // 🌱 EVERY harvest pockets a homestead seed now — flowers too (Trym:
    // "you should be able to plant everything on the homestead just like
    // the park"), so the pouch is one rule instead of a crop-only carve-out
    seedGain(sd.id);
