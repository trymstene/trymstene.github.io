...
const lifeSchema = {
  ...
  properties: {
    ...
    store: { type: 'object', additionalProperties: false, required: ['greet', 'shut', 'needs', 'van', 'sold'],
      properties: { greet: str(lifeFields['store.greet'].note), shut: str(lifeFields['store.shut'].note), needs: str(lifeFields['store.needs'].note), van: str(lifeFields['store.van'].note), staff: str(lifeFields['store.staff'].note),
...
const dutyFields = {
  ...
  'call.deliver': { kind: 'prose', aim: 60, max: 82, note: '📦 The note when the store\u2019s parcel call has come in (rank 3): Pip calls, a parcel waits at the store to be delivered, open until midnight. Starts with Pip\u2019s name.' },
...
export const JOBS = {
  ...
  'town-staff': {
    ...
    fields: {
      ...
      ...Object.fromEntries([['stand', 'big'], ['stand', 'jug'], ['cafe', 'rush'], ['cafe', 'special'], ['cafe', 'keys'], ['condo', 'streak'], ['condo', 'lamps'], ['condo', 'litter'], ['store', 'basket'], ['store', 'deliver'], ['store', 'second'], ['store', 'keys'], ['post', 'fifth'], ['post', 'parcel']].map(([a, k]) => [`unlock.${a}.${k}`, toastLine(80, 'What this rank lets you do at this workplace, in plain words: the new thing first, then what it means. Never a number, a rate or how often it comes (docs/voice.md).', NO_MARKUP)])),
  ...
  'town-deliver': {
    ...
    top: ['picked', 'pickedTwo', 'delivered', 'deliveredOne', 'to'],
    fields: {
      ...
      ...Object.fromEntries(['nib', 'stamp', 'moss', 'bean', 'figjr', 'spinner', 'dot', 'granfig'].map((k) => ['to.' + k, { kind: 'label', max: 30, note: 'The resident and their place, as the parcel line names them: “Name at the place”. The place is where their door is on the square.' }])),
    },
    shape: (d) => Object.keys((d && d.to) || {}).length === 8 ? [] : [{ path: 'to', msg: 'every resident but Pip needs a line: the parcel can be for any of them' }],
  },
  'town-serve': {
    id: 'town-serve',
  ...
  'town-lemon': {
    ...
    top: ['on', 'off', 'idle', 'front', 'receipt', 'go', 'cup', 'left', 'drinks', 'leave', 'big', 'jug'],
    personas: 'town-personas',
    fields: lemonFields,
    shape: lemonShape,
    schema: lemonSchema,
  ...
  'town-cafe': {
    ...
    top: ['on', 'off', 'idle', 'front', 'receipt', 'go', 'cup', 'left', 'drinks', 'leave', 'rush', 'special'],
