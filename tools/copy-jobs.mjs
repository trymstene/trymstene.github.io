...
const lifeFields = {
  ...
  'board.why': { kind: 'prose', aim: 100, max: 120, note: 'The standing notice: what putting things right does for the square — that every fix lifts it, and what a lifted square opens. No number, no rate.' },
  'board.curse': { kind: 'prose', aim: 100, max: 120, note: 'The standing notice on an ordinary day: that some nights the square is cursed, what such a night brings (dark lamps, shut doors, ghosts, things left lying about) — and never when.' },
...
const lifeSchema = {
  ...
  properties: {
    ...
    board: { type: 'object', additionalProperties: false, required: ['title', 'fixes', 'people', 'found', 'next', 'why', 'curse', 'omen', 'night', 'after'],
      properties: { title: str(lifeFields['board.title'].note), fixes: str(lifeFields['board.fixes'].note), people: str(lifeFields['board.people'].note), found: str(lifeFields['board.found'].note),
        next: str(lifeFields['board.next'].note), why: str(lifeFields['board.why'].note), curse: str(lifeFields['board.curse'].note), omen: str(lifeFields['board.omen'].note), night: str(lifeFields['board.night'].note), after: str(lifeFields['board.after'].note) } },
