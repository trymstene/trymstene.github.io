...
import { WEED_GRID, BORDER_SPOTS_N } from './park-weed-grid.js';
...
export class ParkRoom {
  ...
  async garden(request, url) {
    ...
    if (decayed) {
      ...
      const feed = BLOOM_FEED_MAX * stars / (stars + BLOOM_FEED_K);
      bl.v = Math.min(100, Math.max(BLOOM_FLOOR,
        bl.v - hrs * (BLOOM_BASE_DRIFT + BLOOM_WEED_DRAG * wd.list.length - feed)));
      bl.at = now;
    }
    ...
    if (url.pathname === '/garden' && request.method !== 'POST') {
      if (dirty || weedsDirty || decayed || eggsDirty || trashDirty || borderDirty) await persist();
      return json(payload());
    }
    let b = null;
    try { b = JSON.parse(await request.text()); } catch (e) {}
