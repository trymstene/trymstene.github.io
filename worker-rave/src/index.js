...
import { checkLetter, CAPS } from '../../src/lib/letter-gate.js';
...
export class PostRoom {
  ...
  async fetch(request) {
    ...
    if (url.pathname === '/box') {
      ...
      return j({ letters: live.map((x) => ({ id: x.id, from: x.from, at: x.at, text: x.text, read: !!x.read })), unread: live.filter((x) => !x.read).length });
    }
